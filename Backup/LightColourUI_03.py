...
class ColPickTest(QtGui.QGraphicsItem):
    def __init__(self, circleCentreX, circleCentreY, radius,  parent = None):
        ...
        # self.move_restrict_rect = QtCore.QRectF(self.circleCentreX, self.circleCentreY, 200, 200)
        self.move_restrict_rect = QtGui.QGraphicsEllipseItem(40, 37, 2*self.radius, 2*self.radius)
        # set item's rectanglw
    ...
    # def circlePos(self):
    # 	xPos = self.pos().x() - self.circleCentreX
    # 	yPos = self.pos().y() - self.circleCentreY
    # 	distance = math.sqrt((math.pow(xPos,2)) + (math.pow(yPos,2)))
    # 	if distance > self.radius:
    # 		print "Condition"
    # 		self.setPos(self.radius*xPos/distance, self.radius*yPos/distance)
	...
	# 	"""Function to make sure that a position is within a circle limit"""
	# 	x = self.pos().x()
	# 	y = self.pos().y()
	...
	# 		return [x,y]
	# 	else:
	# 		return [radius*x/distance, radius*y/distance]
    ...
    #     	if self.pos().x() > 300:
    #     		print "condition"
    #     		self.setPos(QtCore.QPointF(300,yPos))
    ...
    #     	return QtGui.QGraphicsItem.itemChange(self, change, value)
        	...
        	# xPos = self.pos().x()
        	# yPos = self.pos().y()
        	# print "Item new position " + str(xPos) + ", " +  str(yPos)
        
        

        	# if self.pos().x() > 300:
        	# 	return self.setPos(QtCore.QPointF(300,yPos))
        	# else:
        	# 	return QtGui.QGraphicsItem.itemChange(self, change, value)
...
class ColPickGVItem(QtGui.QGraphicsItem):
    def __init__(self, startx, starty, circleCentreX, circleCentreY, radius,  parent = None):
        super(ColPickGVItem, self).__init__(parent)
        # QtGui.QGraphicsItem.__init__(self)
        self.setFlag(QtGui.QGraphicsItem.ItemIsMovable)
        self.setFlag(QtGui.QGraphicsItem.ItemSendsGeometryChanges)
        self.setCacheMode(self.DeviceCoordinateCache)
        self.setPos(circleCentreX, circleCentreY)
