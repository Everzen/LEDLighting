...
class RigCurveNav():
    ...
    def __init__(startNode, endNode, targNode):
        self.startPos = npVec(self.startNode().pos())
        self.endPos = npVec(self.endNode().pos())
        self.targPos = npVec(self.targNode().pos())
...
class RigCurve(QtGui.QGraphicsItem):
    ...
    def getNodeList(self, controlNodes):
        ...
        for n in controlNodes: self.nodeList.append(weakref.ref(n))
    ...
    def buildSection(self):
        ...
        if len(self.nodeList) >= 3:
            ...
            #Establish distance between starting nodes
            ...
            cP1 = startPoint + self.handlescale*dirVec - sideSwitch*dirDist*perpVec*self.curveSwing*perpSwing 
            cP2 = startPoint + (1-self.handlescale)*dirVec - sideSwitch*dirDist*perpVec*self.curveSwing*perpSwing 
            ...
            self.startNode().setBezierHandles(cP1, 1)
            self.endNode().setBezierHandles(cP2, 0)
            ...
            cPNext = (endPoint - cP2)*dirDist/targNodeDist  + endPoint

            self.endNode().setBezierHandles(cPNext, 1)
            print "End Node Handle Test : " + str(self.endNode().getBezierHandles(0))
            print "End Node Handle Test : " + str(self.endNode().getBezierHandles(1))
            # print "CP1 : " + str(cP1)
            # print "CP2 : " + str(cP2)
